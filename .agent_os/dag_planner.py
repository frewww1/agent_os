...
无第三方依赖，供 dag.py（调度 agent CLI）与 dag_engine.py 共用。
...
def topo_order(steps: list[dict]) -> list[str]:
    """Kahn 拓扑排序（移植自 dag_engine.get_execution_order）。

    steps: [{"id": ..., "depends_on": [...]}, ...]
    按 steps 原始顺序入队 in_degree==0 的节点，保证输出稳定。
    检测环：若排序结果不覆盖全部节点，抛 ValueError。
    depends_on 中不存在的 id 会被过滤（防脏数据）。
    """
    ids = [s["id"] for s in steps]
    id_set = set(ids)
    in_degree = {
        s["id"]: len([d for d in s.get("depends_on", []) if d in id_set])
        for s in steps
    }
