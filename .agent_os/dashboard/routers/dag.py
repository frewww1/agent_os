...
async def start_dag(req: DagStartRequest):
    ...
    system_prompt = (
        ...
        f"2. 对每个就绪节点创建子 agent，传入节点的 prompt、goal、supervisor，"
        f"并在 prompt 末尾追加 step_id=<节点的 id>\n"
        f"3. 派发完所有就绪节点后结束对话，等 OS resume\n"
        f"4. resume 后 `python .agent_os/dag.py --mark-done <id>`，回到第 1 步\n"
        f"5. --ready 返回空时 `python .agent_os/report.py --result \"全部完成\"`\n\n"
        f"⚠️ 不要用 curl、Bash 调 API，用创建子 agent 的方式来派发。"
    )
