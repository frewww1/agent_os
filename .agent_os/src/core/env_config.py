...
def build_agent_env(agent_id: str, project_root: str, port: int,
                    workspace_name: str | None = None,
                    env_extras: dict | None = None) -> dict:
    """构建子进程环境变量（启动 + continue 共用）。"""
    env = os.environ.copy()
    env.update({
        "AGENT_OS_AGENT_ID": agent_id,
    ...
    if env_extras and "AGENT_OS_WORKSPACE" in env_extras:
        ...
        logger.info(f"[{agent_id[:8]}] Inherited workspace from parent: {env_extras['AGENT_OS_WORKSPACE']}")
    # 根 agent 新建 workspace
    elif "AGENT_OS_WORKSPACE" not in env:
        dir_name = sanitize_workspace_name(workspace_name) if workspace_name else agent_id
        ...
        logger.info(f"[{agent_id[:8]}] {action} workspace: {workspace_path}")
